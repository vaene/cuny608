...
// Gradient endpoints matched to the legend bar
// Light end: #f8faf0  (248, 250, 240)
// Dark end:  #5ca36c  (92, 163, 108)
function getHeatColor(value: number, min: number, max: number): string {
  const range = max - min || 1;
  const t = Math.max(0, Math.min(1, (value - min) / range));

  const red = Math.round(248 - t * (248 - 92));
  const green = Math.round(250 - t * (250 - 163));
  const blue = Math.round(240 - t * (240 - 108));
...
export default function DensityAnalysisPage() {
  ...
  const [orderedRegions, setOrderedRegions] = useState<RegionSummary[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        for (const region of REGION_ORDER) {
          const regionCells = realCells.filter((c) => c.region === region);
          if (regionCells.length === 0) continue;
          const avgReal = regionCells.reduce((sum, c) => sum + c.real, 0) / regionCells.length;
          const avgNominal = regionCells.reduce((sum, c) => sum + c.nominal, 0) / regionCells.length;
          const cola = regionCells[0].cola;
          regionSummaries.set(region, {
            region,
            cola,
            avgNominal,
            avgReal,
            relation: cola < 100 ? "lower" : cola > 100 ? "higher" : "equal"
          });
        }

        const sortedRegions = Array.from(regionSummaries.values()).sort((a, b) => b.avgNominal - a.avgNominal);

        setCells(realCells);
        setOrderedRegions(sortedRegions);
        setStatus("");
      } catch (error) {
  ...
  const relationOrder: RegionSummary["relation"][] = ["higher", "equal", "lower"];
  const relationGroups = relationOrder
    .map((relation) => ({
      relation,
      regions: orderedRegions.filter((summary) => summary.relation === relation)
    }))
          ...
          {status ? (
            ...
            <div className="space-y-4">
              <div
                ...
                style={{
                  gridTemplateColumns: `minmax(220px, 1.25fr) repeat(${ROLE_ORDER.length}, minmax(120px, 1fr))`
                }}
              >
                ...
                {relationGroups.map((group) => (
                  <div key={group.relation} className="contents">
                    <RelationHeader relation={group.relation} />
                    {group.regions.map((summary) => (
                      <RegionRow
                        key={summary.region}
                        summary={summary}
                        roleOrder={ROLE_ORDER}
                        cells={cells.filter((cell) => cell.region === summary.region)}
                        minReal={minReal}
                        maxReal={maxReal}
                      />
                    ))}
              ...
              <div className="flex items-center justify-between gap-4 rounded-xl bg-gray-50 px-4 py-3 text-xs text-gray-600">
                ...
                <div className="h-3 flex-1 rounded-full bg-gradient-to-r from-[#5ca36c] via-[#b6dcb3] to-[#f8faf0]" />
        ...
        <div className="mt-8 space-y-4 border-t border-gray-200 pt-6">
          <div className="rounded-lg bg-blue-50 p-4">
            ...
            <p className="mt-2 text-sm text-blue-800">
              ...
              <p className="mt-2 text-sm text-blue-800">
                The thin section headers show whether nominal salary sits below, matches, or exceeds adjusted income.
              </p>
          </div>
...
function RelationHeader({ relation }: { relation: RegionSummary["relation"] }) {
  const label =
    relation === "lower"
      ? "Nominal < Adjusted Income"
      : relation === "higher"
        ? "Nominal > Adjusted Income"
        : "Nominal = Adjusted Income";
...
function RegionRow({
  ...
  const { region, cola, avgNominal, avgReal } = summary;

  return (
    <>
      <div className="flex items-center rounded-l-xl border border-gray-100 bg-gray-50 px-3 py-4 text-sm font-semibold text-gray-900">
        <div className="w-full">
          <div className="text-base">{region}</div>
          <div className="mt-1 text-xs font-normal text-gray-600">COLA {cola} · Avg nominal {formatCurrency(avgNominal)}</div>
          <div className="mt-1 text-[11px] uppercase tracking-[0.16em] text-gray-500">
            Real average {formatCurrency(avgReal)}
          </div>
        </div>
      </div>
      ...
      {roleOrder.map((role) => {
        ...
        if (!cell) {
          return <div key={`${region}-${role}`} className="rounded-r-xl border border-gray-100 bg-gray-50 p-4" />;
        }

        return (
          <div
            key={`${region}-${role}`}
            className="rounded-r-xl border border-white/60 p-4 text-center shadow-sm"
            style={{
