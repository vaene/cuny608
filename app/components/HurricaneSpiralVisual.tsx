...
const parseHurricaneCsv = (text: string): SpiralEntry[] => {
  ...
  const monthIndex = headerLower.indexOf('month');
  const usaWindIndex = headerLower.indexOf('usa_wind');
  const wmoWindIndex = headerLower.indexOf('wmo_wind');
  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    const month = Number.parseInt(row[monthIndex], 10);
    if (!Number.isFinite(year) || !Number.isFinite(month)) continue;
    if (year < START_YEAR || year > END_YEAR) continue;
    const rawUsa = row[usaWindIndex]?.trim();
    const rawWmo = row[wmoWindIndex]?.trim();
    const wind = Number.parseFloat(rawUsa || '') || Number.parseFloat(rawWmo || '');
    if (!Number.isFinite(wind)) continue;
    const key = `${year}-${month}`;
    const prev = monthlyMax.get(key) ?? -Infinity;
    if (wind > prev) monthlyMax.set(key, wind);
  }
  ...
  for (let year = START_YEAR; year <= END_YEAR; year += 1) {
    for (let month = 1; month <= 12; month += 1) {
      const key = `${year}-${month}`;
      const value = monthlyMax.get(key) ?? 0;
...
const formatWind = (value: number) => `${Math.round(value)} kt`;
const formatWindLabel = (value: number | null) => (value === null ? '—' : formatWind(value));
      ...
      try {
        const dataUrls = (() => {
          if (typeof window === 'undefined') {
            return ['/data/ibtrac_hurricane_data.csv'];
          }
          const paths = new Set<string>();
          const segments = window.location.pathname.split('/').filter(Boolean);
          for (let i = segments.length; i >= 0; i -= 1) {
            const prefix = i === 0 ? '' : `/${segments.slice(0, i).join('/')}`;
            paths.add(`${window.location.origin}${prefix}/data/ibtrac_hurricane_data.csv`);
          }
          paths.add(`${window.location.origin}/data/ibtrac_hurricane_data.csv`);
        ...
        const parsed = parseHurricaneCsv(text);
        if (!cancelled) {
          if (parsed.length === 0) {
            setDataError('ibtrac_hurricane_data.csv loaded but contained no usable rows.');
          }
          setData(parsed);
          setLoading(false);
        }
      } catch (err) {
        console.error('Failed to load hurricane data', err);
        if (!cancelled) {
          setData([]);
          setDataError('Failed to load ibtrac_hurricane_data.csv.');
          setLoading(false);
        }
      }
    };
  ...
  if (loading) {
    return (
      <div className={`w-full ${compact ? 'h-[580px]' : 'h-[720px]'} bg-black flex items-center justify-center rounded-lg border border-slate-700`}>
        <div className="text-slate-300">Loading hurricane data...</div>
      </div>
    );
  }
    ...
    <div className="w-full">
      <div className="flex flex-col lg:flex-row gap-4 items-stretch">
        <div className="w-full lg:w-48 flex flex-col gap-3">
          ...
          <div className="text-xs text-slate-500 leading-relaxed">
            Monthly max wind speed from IBTrACS (USA/WMO).
          </div>
        </div>
      ...
      <div className="flex-1 bg-black border border-slate-700 rounded-lg shadow-xl relative overflow-hidden">
        ...
        <div className="w-full lg:w-56 flex flex-col gap-3">
          <div className="text-slate-200 text-sm font-semibold">Hurricane Spiral</div>
          <div className="text-xs text-slate-400">
            Styled after the NASA spiral, driven by IBTrACS wind.
          </div>
          <div className="mt-3">
