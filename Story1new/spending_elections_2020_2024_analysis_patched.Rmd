...
We compare federal obligations per capita across states using a Biden-era average (FY2021–FY2024) versus a pre-Biden baseline (FY2017–FY2020), then test how those changes align with 2024 presidential margins.
...
# IIJA Funding Snapshot (as of March 2023)

```{r iija-load, message=FALSE}
IIJA_XLSX_PATH <- "IIJA FUNDING AS OF MARCH 2023.xlsx"

iija <- NULL
if (file.exists(IIJA_XLSX_PATH)) {
  iija <- readxl::read_excel(IIJA_XLSX_PATH) |>
    janitor::clean_names() |>
    transmute(
...
# State/territory name -> abbreviation lookup
...
# IIJA Funding Snapshot (March 2023)

This chart uses the **IIJA funding snapshot (as of March 2023)** to provide a program-specific view alongside the all-program USAspending totals. The snapshot is not a substitute for total obligations, but it offers a focused look at infrastructure-specific allocations, which is why other datasets are used for the broader spending and election comparisons.

```{r iija-snapshot, fig.height=10}
if (!is.null(iija) && nrow(iija) > 0 && exists("pop")) {

  iija_plot <- iija |>
    filter(state %in% c(state.abb, "DC")) |>
    left_join(pop |>
                filter(fiscal_year == 2023) |>
                select(state, pop_2023 = pop),
              by = "state") |>
    mutate(
      iija_total = iija_total_bil * 1e9,
      iija_pc = iija_total / pop_2023
    ) |>
    left_join(pres2020 |> select(state, winner_2020), by = "state") |>
    arrange(desc(iija_pc)) |>
    mutate(state = factor(state, levels = state))

  ggplot(iija_plot, aes(x = iija_pc, y = state, fill = winner_2020)) +
    geom_col(width = 0.75) +
    scale_party_fill(guide = "none") +
    scale_x_continuous(labels = label_dollars1) +
    labs(
      title = "IIJA funding per capita by state (March 2023 snapshot)",
      subtitle = "Program-specific snapshot; per-capita uses 2023 population",
      x = "IIJA funding per capita ($)",
      y = NULL
    ) +
    theme_low_ink() +
    theme(
  ...
  message("IIJA snapshot not available. Ensure the Excel file is present.")
}
```

### Key Observations

1. **Program-specific contrast:** This IIJA snapshot shows infrastructure allocations only, which can diverge from overall federal obligations shown in the later charts.

2. **Per-capita framing:** Using 2023 population normalizes for state size, highlighting smaller states with higher infrastructure dollars per resident.

# Bar Charts (FY2024): Spending and Spending Per Capita

These charts use a **horizontal bar** layout for readable state labels, and follow the low-ink theme set above. The IIJA file is a program-specific snapshot, so USAspending totals are included to capture the full federal spending picture needed for the broader analysis.
...
1. **IIJA as the trigger:** The IIJA snapshot motivated the broader inquiry, but it only covers one program. We pull **total federal obligations** from USAspending to see whether the IIJA pattern holds in overall spending.
...
1. **Why per‑capita is clearer:** Per‑capita spending controls for population size, revealing states that look small in total dollars but large **per resident**—useful when comparing to a per‑capita IIJA snapshot.

2. **Per‑capita pitfalls:** Very small population states and DC can show extreme values because the denominator is small. These outliers are real but can **exaggerate swings**, so they should be read alongside totals and the IIJA snapshot.
...
# Additional Evidence: Does 2020 Voting Predict Spending Deltas?

This section adds multiple robustness views to test whether post‑2020 spending shifts align with 2020 outcomes and whether those shifts predict 2024 results.

## Difference in Means (Delta by 2020 Winner)

```{r diff-in-means, fig.height=4}
diff_means <- delta_12_1 |>
  group_by(winner_2020) |>
  summarize(
    mean_delta = mean(delta_biden_vs_pre_pc, na.rm = TRUE),
    sd_delta = sd(delta_biden_vs_pre_pc, na.rm = TRUE),
    n = n(),
    se = sd_delta / sqrt(n),
    .groups = "drop"
  )

ggplot(diff_means, aes(x = winner_2020, y = mean_delta, fill = winner_2020)) +
  geom_col(width = 0.6, show.legend = FALSE) +
  geom_errorbar(aes(ymin = mean_delta - 1.96 * se, ymax = mean_delta + 1.96 * se),
                width = 0.2, linewidth = 0.3) +
  scale_party_fill(guide = "none") +
  scale_y_continuous(labels = label_dollars1) +
  labs(
    title = "Average spending delta by 2020 winner (with 95% CI)",
    x = NULL,
    y = "Mean delta per capita ($)"
  ) +
  theme_low_ink()
```

## Distribution of Deltas (Boxplot by 2020 Winner)

```{r boxplot-delta, fig.height=4}
ggplot(delta_12_1, aes(x = winner_2020, y = delta_biden_vs_pre_pc, fill = winner_2020)) +
  geom_boxplot(width = 0.5, outlier.alpha = 0.4, show.legend = FALSE) +
  scale_party_fill(guide = "none") +
  scale_y_continuous(labels = label_dollars1) +
  labs(
    title = "Distribution of spending deltas by 2020 winner",
    x = NULL,
    y = "Delta per capita ($)"
  ) +
  theme_low_ink()
...
## Binned Scatter (Spending Delta vs 2024 Margins)

```{r binned-scatter, fig.height=4}
bins <- analysis |>
  mutate(bin = ntile(delta_biden_vs_pre_pc, 8)) |>
  group_by(bin) |>
  summarize(
    avg_delta = mean(delta_biden_vs_pre_pc, na.rm = TRUE),
    avg_pres_margin = mean(pres_margin_dem, na.rm = TRUE),
    avg_house_margin = mean(house_margin_dem, na.rm = TRUE),
    .groups = "drop"
  )

ggplot(bins, aes(x = avg_delta, y = avg_pres_margin)) +
  geom_point() +
  geom_line() +
  scale_x_continuous(labels = label_dollars1) +
  scale_y_continuous(labels = percent) +
  labs(
    title = "Binned averages: spending delta vs 2024 presidential margin",
    x = "Avg delta per capita ($)",
    y = "Avg 2024 presidential margin"
  ) +
  theme_low_ink()
```

```{r binned-scatter-house, fig.height=4}
ggplot(bins, aes(x = avg_delta, y = avg_house_margin)) +
  geom_point() +
  geom_line() +
  scale_x_continuous(labels = label_dollars1) +
  scale_y_continuous(labels = percent) +
  labs(
    title = "Binned averages: spending delta vs 2024 House margin",
    x = "Avg delta per capita ($)",
    y = "Avg 2024 House margin"
  ) +
  theme_low_ink()
```

## Residuals by State (Presidential Model)

```{r residuals-pres, fig.height=6}
pres_lm2 <- lm(pres_margin_dem ~ delta_biden_vs_pre_pc, data = analysis)

resid_df <- analysis |>
  mutate(resid = resid(pres_lm2)) |>
  arrange(resid) |>
  mutate(state = factor(state, levels = state))

ggplot(resid_df, aes(x = resid, y = state)) +
  geom_col(fill = "#999999") +
  scale_x_continuous(labels = label_pct1) +
  labs(
    title = "Residuals from presidential model (state-level)",
    x = "Residual (margin points)",
    y = NULL
  ) +
  theme_low_ink()
```
...
```{r spearman, message=FALSE}
spearman_pres <- cor(analysis$delta_biden_vs_pre_pc, analysis$pres_margin_dem, method = "spearman", use = "complete.obs")
spearman_house <- cor(analysis$delta_biden_vs_pre_pc, analysis$house_margin_dem, method = "spearman", use = "complete.obs")

cat("Spearman (Pres):", round(spearman_pres, 3), "\\n")
cat("Spearman (House):", round(spearman_house, 3), "\\n")
```

**Key Result:** Across these checks, the evidence still points to weak‑to‑moderate association at best, with large dispersion and overlapping distributions across 2020 winners.
...
**IIJA funding snapshot (March 2023)**  
Local file: [IIJA FUNDING AS OF MARCH 2023.xlsx](IIJA FUNDING AS OF MARCH 2023.xlsx)  
Source: User-provided (snapshot file)  
Processing: Loaded from Excel, mapped to state abbreviations, and converted to per-capita values using 2023 population.
