...
export default function ResidualsPage() {
  ...
  const chartDescription =
    'Residuals show how far each state deviates from the fitted relationship between spending delta and 2024 presidential margin. Large residuals indicate the model does not explain the state well.';
  const keyObservations =
    'Key observations: Several states deviate strongly from the fitted line, reinforcing that spending deltas are not a reliable predictor of 2024 outcomes.';
  ...
  useEffect(() => {
    async function main() {
      try {
        const [popText, states, presCsv] = await Promise.all([
        ...
        const rows: { state: string; delta: number; pres: number }[] = [];
        for (const st of states) {
          ...
          if (pres === undefined) continue;
          rows.push({ state: st, delta, pres });
        }

        const xBar = rows.reduce((s, r) => s + r.delta, 0) / rows.length;
        const yBar = rows.reduce((s, r) => s + r.pres, 0) / rows.length;
        const ssX = rows.reduce((s, r) => s + (r.delta - xBar) ** 2, 0);
        const ssXY = rows.reduce((s, r) => s + (r.delta - xBar) * (r.pres - yBar), 0);
        const slope = ssXY / ssX;
        const intercept = yBar - slope * xBar;

        const residuals = rows.map((r) => ({
          state: r.state,
          resid: r.pres - (slope * r.delta + intercept)
        }));
        residuals.sort((a, b) => a.resid - b.resid);

        buildChart(residuals);
        setStatus('');
      } catch (err) {
        console.error(err);
    ...
    function buildChart(residuals: { state: string; resid: number }[]) {
      if (!canvasRef.current) return;
      if (chartRef.current) chartRef.current.destroy();
      const config: ChartConfiguration<'bar', number[], string> = {
        type: 'bar',
        data: {
          labels: residuals.map((r) => r.state),
          datasets: [
            {
              label: 'Residual',
              data: residuals.map((r) => r.resid),
        ...
        options: {
          ...
          scales: {
            ...
            y: { ticks: { callback: (v: string | number) => formatPercent(Number(v)) }, grid: { display: false } }
          }
        }
      };
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="mx-auto max-w-6xl rounded-lg bg-white p-5 pb-16 shadow flex flex-col h-screen">
        <div className="mb-4">
          <h1 className="text-xl font-bold text-gray-900">Residuals by State</h1>
          <p className="text-sm text-gray-500 mt-1">Presidential model residuals</p>
        </div>
        ...
        <Link
          ...
          aria-label="Go to house scatter"
        >
        ...
        <div className="fixed bottom-2 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 rounded-full border border-gray-200 bg-white/65 px-4 py-1 text-xs text-gray-600 shadow-md backdrop-blur scale-75 origin-bottom">
          <span className="font-semibold text-gray-900">9 / 11</span>
